...
>Table (\#tab:tab-soil-1). Thermal propertie8 of soils and their components (from Montieth, 1973).
...
```{r fig-soil-10, fig.cap='Variation of temperature with depth for a sand soil. A value of $k=0.042calcm^{-1}sec^{-1\\circ} C^{-1}$ and $C=0.5cal cm^{-3\\circ} C^{-1}$ has been assumed resulting in a damping depth D=15.2cm'}
...
3. Refer to Figure \@ref(fig:fig-soil-2).  
    ...
    (b) Estimate $A(0)$.  
    (c) Calculate diffusivity $(K)$. Was the soil wet or dry (see Table \@ref(tab:tab-soil-1), or Figure \@ref(fig:fig-soil-15) in problem 4)?
...
Using the identity $sm(x+4)=\sin x\cos y+\cos x\sin y$  
