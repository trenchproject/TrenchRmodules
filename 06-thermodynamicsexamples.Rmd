---
pagetitle: Thermodynamics Applications
---
...
## Preface {#thermodynamicsexamples-preface}
...
## Introduction {#thermodynamicsexamples-intro}
...
### Work {#thermodynamicsexamples-work}
...
(\#eq:thermex-4)
\end{equation}
By substituting equations \@ref(eq:thermex-3) and \@ref(eq:thermex-4) into equation \@ref(eq:thermex-2) we express the First Law as
\begin{equation}
C_V dT = -P dV
(\#eq:thermex-5)
\end{equation}
...
(\#eq:thermex-6)
\end{equation}
Since our goal is to express the temperature change as a function of height or elevation we will eliminate $P dV$ from equation \@ref(eq:thermex-5) by substituting into equation \@ref(eq:thermex-6) which yields 
\begin{equation}
C_V dT = -R dT + V dP
(\#eq:thermex-7)
\end{equation}
...
where
<ul class="list-unstyled">
<li> $g$ is the acceleration of gravity, </li>
<li> $m$ is molecular mass of air, </li>
<li> and $V$ is the volume of one mole. </li>
</ui>

Combining equations \@ref(eq:thermex-8) and \@ref(eq:thermex-9) and rearranging we have
\begin{equation}
V dP = -g m dh
(\#eq:thermex-10)
\end{equation}

Substituting equation \@ref(eq:thermex-10) into \@ref(eq:thermex-7) and rearranging we have $(c_V + R) dT = -g m d$ the desired result; a formula that tells us how temperature changes with height. (Also recall that for an ideal diatomic gas that the heat capacity at constant volume $C_V = 2.5 R.$)
...
### Examples of Heat Energy Exchange {#thermodynamicsexamples-exchange}
...
where
<ul class="list-unstyled">
<li> $\Delta U$ = change in internal energy ($W m^2$, watts per square meter), </li>
<li> $Q_a$ = radiation absorbed by the system surface($W m^{-2}$), </li>
<li> $P$ = photosynthesis ($W m^{-2}$), </li>
<li> $M$ = basal metabolism($W m^{-2}$), </li>
<li> $Q_e$ = radiation emitted by the system surface ($W m^{-2}$), </li>
<li> $L$ = latent heat of evaporation ($J kg^{-1}$), </li>
<li> $E$ = evaporation flux density ($kg m^2 s^{-1}$), </li>
<li> $G$ = conduction ($W m^{-2}$), </li>
<li> and $C$ = convection ($W m^{-2}$). </li>
</ul>

Equation \@ref(eq:thermex-12) is a general heat flow equation. In most cases except for water vapor flux $LE$, mass exchange can be neglected. Similarly, the system's internal sources of heat energy, photosynthesis and metabolism, are also very small components of the heat energy budget and can usually be ignored. We note, however, there are important exceptions of which the largest group includes endothermic vertebrates, birds and mammals. The other terms $Q_a$, $Q_e$, $LE$, $C$, $G$ of equation \@ref(eq:thermex-12) represent the basic heat transfer processes that were described in the first module entitled "The First Law of Thermodynamics for Ecosystems."

The first example we wish to consider is the heat energy budget of the earth. For the simplicity of the following example, we will for the moment put asie anthropogenic global warming, and assume the average temperature of both the earth and the atmosphere are not changing with time. Thus, we assume $\Delta U = 0$. Therefore the sum of the heat fluxes is zero and
\begin{equation}
0 = Q_a - Q_e -LE -C 
(\#eq:thermex-13)
\end{equation}
where
<ul class="list-unstyled">
<li> $Q_a$ = radiation absorbed from the sun (left half of Fig. \@ref(fig:fig-thermex-2)),
<li> $Q_e$ = energy emitted by the atmosphere and earth,
<li> $LE$ = heat loss by evaporation or latent heat,
<li> and $C$ = convection or sensible heat.
</ul>
...
```{r fig-thermex-1, echo=FALSE, fig.height=3, fig.show = "hold", out.width = "100%", fig.align = "default",, fig.cap='Relative net transfer of heat within the earth-atmosphere-space system. From Lowry, W.P. 1969. P. 27.'}
...
where
<ul class="list-unstyled">
<li> $\Delta U$ = net change in internal energy of the stream ($W m^{-2}$),
<li> $Q_{NR}$= net thermal radiation flux ($Q_a - Q_e$)($W m^{-2}$),
<li> $LE$ = evaporative flux ($W m^{-2}$),
<li> $G$ = conductive flux ($W m^{-2}$),
<li> and $C$ = convective flux ($W m^{-2}$).
</ul>
...
where
<ul class="list-unstyled">
<li> $Q_a$ = radiant energy absorbed by the leaves ($W m^{-2}$), </li>
<li> $Q_e$ = radiation emitted by the leaves ($W m^{-2}$), </li>
<li> $C$ = convection flux ($W m^{-2}$), </li>
<li> and $LE$ = evaporative flux ($W m^{-2}$). </li>
</ul>

A positive sign indicates energy is being added to the system. Equation \@ref(eq:thermex-15) describes completely the heat balance of a leaf. This is very important since the heat energy balance determines leaf temperature which affects the photosynthetic activity and shape of the leaf. A more complete analysis of the leaf system and the biological implications will be given in another module (Gates). In this module, it is most important to realize that we can write down the heat energy budget of the leaf and that there are four basic components to the heat balance: $Q_a$, $Q_e$, $C$, and $LE$. 
...
where
<ul class="list-unstyled">
<li> where $Q_a$ = radiant absorbed ($W m^{-2}$), </li>
...
### The First Law Generalized to Include Mass Flow  {#thermodynamicsexamples-firstlaw}
...
## Problem Set {#thermodynamicsexamples-problems}
...
```{r fig-thermex-9, echo=FALSE, fig.height=4, ig.show = "hold", out.width = "75%", fig.align = "default", fig.cap='Figure for problen 2. Isotherms in degrees Celsius for elevation vs. latitude. The mean annual temperature at any elevation is calculated assuming a dry adiabatic lapse rate. Mean annual temperature at growund level are taken from Miller and Thompson 1970.'}
...
5.    
a. The mass exchange is $2.22 \times 10^{-6} kg s^{-1}$. Multiplied by a temperature difference of 30°C and a specific heat of $1010 J kg^{-1^\circ}C^{-1}$ for air at 20°C (average value) yields a net loss of $0.067 W$. This is approximately 2.5% of the metabolic rate of a bird of this mass.

b. Heat loss = mass flux $\times$ specific heat $\times$ temperature difference
$$10\frac{\mbox{gallons}}{\mbox{day}}\times\frac{8.34lb.}{\mbox{gal}}\times\frac{1kg}{2.205lb.}\times\frac{1\mbox{day}}{86400s}\times18^\circ C\times4180Jkg^{-1\circ}C=32.9W$$
...
```{r fig-thermex-10a, echo=FALSE, fig.height=4, ig.show = "hold", out.width = "100%", fig.align = "default", fig.cap='Search and capture rates as a function of temperature for an endotherm (---) and an ectotherm (- $\\cdot$ -).'}
knitr::include_graphics('figures/fig-thermex-10a.png')
```

```{r fig-thermex-10b, echo=FALSE, fig.height=4, ig.show = "hold", out.width = "100%", fig.align = "default",fig.fullwidth=FALSE, fig.cap='Digestion, absorption and chemical processing rates as a function of temperature for an endotherm (---) and an ectotherm (- $\\cdot$ -).'}
...
## Literature Cited {#thermodynamicsexamples-litcited}
...
## Bibliography {#thermodynamicsexamples-bibliography}

### General Texts and Papers on Energy Budgets {#thermodynamicsexamples-gen}
...
### Meteorology {#thermodynamicsexamples-meteorology}
...
### Thermodynamics and Heat Transfer {#thermodynamicsexamples-themandheat}
