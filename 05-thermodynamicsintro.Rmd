...
```{r fig-thermintro-1, echo=FALSE, fig.height=5, fig.width=5, fig.cap='A pressure-temperature diagram for a substance such as water. The triple point is the point of intersection of the sublimation, fusion and vaporization curves. From Zemansky, M.W. and H.C. Van Ness. 1966. P. 200.'}
...
```{r fig-thermintro-2, echo=FALSE, fig.height=4, fig.fullwidth=FALSE, fig.cap='Electromagnetic spectrum as a function of both frequency (f) (Hertz or cycles sec) and wavelength ($\\lambda$) (meters). From Byers, H.R. 1974. General Meteorology. P. 8.'}
...
Before considering work, the last term of eq. \@ref(eq:2), we will define the heat capacity. It is
\begin{equation}
...
(\#eq:4)
\end{equation}
In eq. \@ref(eq:3) $V$ means that the process of adding heat to the system is carried out at constant volume. $C_V$ represents the proportionality constant for the change in temperature per unit amount of heat added. Similarly in eq. \@ref(eq:4) $P$ stands for a constant pressure process.
...
```{r fig-thermintro-3, echo=FALSE, fig.height=4, fig.fullwidth=FALSE, fig.cap='Ideal gas contained in a piston chamber. $\\Delta Q$ will be zero if the wall surrounding the system is perfectly insulated (adiabatic). $\\Delta W$ represents the change it work accomplished by moving the piston.'}
knitr::include_graphics('figures/fig-thermintro-3.png')
```

```{r fig-thermintro-4, echo=FALSE, fig.height=4, fig.fullwidth=FALSE, fig.cap='Pressure-volume diagram. A cycle is composed of going from 1 to 2, 2 to 3 and so on back to 1. When the cycle is composed of isothermal and adiabatic processes it is called a Carnot Cycle. Other cycles can involve constant pressure or constant volume steps. Note that $T_1$ is greater than $T_2$.'}
...
(\#eq:9)
\end{equation}
If eq. \@ref(eq:9) applies then it is possible to calculate $\Delta U$, $Q$ and $W$ for any path. $W$ can be evaluated numerically, $\Delta V$ can be calculated from the change in temperature (from the Ideal Gas Law) and $Q$ is gotten by subtraction.
...
(c) All the potential energy is now kinetic energy which now totals 988 J.  
