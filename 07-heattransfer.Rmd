...
```{r fig-heattrans-2, echo=FALSE, fig.height=4, fig.show = "hold", out.width = "100%", fig.align = "default", fig.cap='Surface activity of *Pogonomyrmex badius* in relation to temperature of the ground surface and time of day. Active ants on the mound are indicated by crosses; inactive mounds are indicated by open circles; half-closed circles denote very slight activity. The months represented, beginning with the highest curve at 12 noon, are June, July, May, October and February. (From Colley, F.Bf. and J.B. Gentry, 1964, p. 224.)'}
...
Table: (\#tab:table-heattrans-1). Long Wave Emissivities (percent, from Monteith, J.L., 1973, p. 68.)
...
Table: (\#tab:table-heattrans-2). Reflectivities of Biological Materials for Solar Radiation (from Monteith, J.L., 1973, pp. 66-67.)</caption>
...
As an example of radiation flux, let us calculate the radiant energy emitted by the cactus, *Opuntia rufida*. To do this, we must specify the emissivity $\varepsilon$ and the surface temperature $T_s$. In Table \@ref(tab:table-heattrans-1), $\varepsilon = 0.977$ for *O. rufida* and, if we take $T_s = 10 ^{\circ}C$, then:
\begin{equation}
...
<caption> (\#tab:table-heattrans-3). Thermal properties of natural materials.</caption>
...
```{r fig-heattrans-7, echo=FALSE, fig.height=4, fig.fullwidth=FALSE, fig.cap='Schematic view of a unit of soil, showing heat inflow from above and heat outflow below, leading to derivation of Eq. \@ref(eq:heattrans-22). (From Lowry, W.P. 1969. P. 51)'}
...
```{r fig-heattrans-ps2a, echo=FALSE, fig.height=4, fig.show = "hold", out.width = "100%", fig.align = "default", fig.cap='Figure for problem 2. Emitted radiation as a function of temperature (T = 280 K)'}
...
T = 280
y = function(x){(2 * pi * h * cl^2/((x*10^-6)^5*(exp(h*cl/kb/T/(x*10^-6))-1)))/10^7}
z = function(x){(2 * pi * h * cl^2/((x*10^-6)^5*(exp(h*cl/kb/T/(x*10^-6))-1)))*(x*10^-6)^2/cl*10^11}

par(mar = c(5, 5, 3, 5))
par(las=1)
curve(y, from = 1, to = 200, n = 500, log = "x", col = "blue", lwd = 2, 
      xlab = expression(lambda ~ (m ~ x ~ 10^6)), 
      ylab = expression(E[lambda] ~ (J ~ m^{-3} ~ s^{-1} ~ x ~ 10^7)), xaxt = "n")
curve(z, from = 1, to = 200, n = 500, log = "x", col = "red", lwd = 2, add = TRUE)
axis(side = 4, at = seq(from = 0, to = 2, length.out = 5), labels = seq(from = 0, to = 20, length.out = 5))
axis(side = 1, at = c(1, 10, 100), labels = c(1, 10, 100))
par(las=0)
axis(side = 4, at = 1.1, labels = expression(E[f] ~ (J ~ m^{-2} ~ x ~ 10^{-12})), gap.axis = 5, pos = 500, tick = FALSE)
legend("topright", lty = 1, col = c("blue", "red"), legend = c(expression(E[lambda]), expression(E[f])))
```

```{r fig-heattrans-ps2b, echo=FALSE, fig.height=4, fig.show = "hold", out.width = "100%", fig.align = "default", fig.cap='Figure for problem 2. Emitted radiation as a function of temperature (T = 6000 K)'}

T = 6000
y = function(x){(2 * pi * h * cl^2/((x*10^-6)^5*(exp(h*cl/kb/T/(x*10^-6))-1)))/10^13}
z = function(x){(2 * pi * h * cl^2/((x*10^-6)^5*(exp(h*cl/kb/T/(x*10^-6))-1)))*(x*10^-6)^2/cl*10^8}

par(mar = c(5, 5, 3, 5))
par(las=1)
curve(z, from = 0.1, to = 10, n = 300, log = "x", col = "red", lwd = 2,
      xlab = expression(lambda ~ (m ~ x ~ 10^6)), 
      ylab = expression(E[lambda] ~ (J ~ m^{-3} ~ s^{-1} ~ x ~ 10^13)), xaxt = "n")
curve(y, from = 0.1, to = 10, n = 300, log = "x", col = "blue", lwd = 2, add = TRUE)
axis(side = 4, at = seq(from = 0, to = 12, length.out = 7), labels = seq(from = 0, to = 12, length.out = 7))
axis(side = 1, at = c(0.1, 1, 10), labels = c(0.1, 1, 10))
par(las=0)
axis(side = 4, at = 6, labels = expression(E[f] ~ (J ~ m^{-2} ~ x ~ 10^{-8})), pos = 18, tick = FALSE)
legend("topright", lty = 1, col = c("blue", "red"), legend = c(expression(E[lambda]), expression(E[f])))
...
3a. Equation \@ref(eq:heattrans-16) is

```{r fig-heattrans-ps3, echo=FALSE, fig.height=4, fig.show = "hold", out.width = "100%", fig.align = "default"}

par(mar = c(5, 5, 3, 5))
y = function(T){(2.50 * 10^6 - 2.38 * 10^3 * T)/10^6}
curve(y, from = 0, to = 50, n = 2, yaxt = "n", xlab = "T (°C)", ylim = c(2.3, 2.6),
      ylab = expression("Latent heat of evaporation " ~ (J ~ kg^{-1} ~ x ~ 10^6)), main = expression("L(T) = 2.50 x" ~ 10^6 ~ "- 2.38 x" ~ 10^3 ~ T))
curve(2.43 + x * 0, from = 0, to = 50, add = TRUE, lty = 2)
axis(side = 2, at = c(2.3, 2.4, 2.5, 2.6), labels = c(2.3, 2.4, 2.5, 2.6))
text(x = 48, y = 2.45, labels = "2.43")
...
\begin{align*}
R_v&=176.3\exp(0.07T_a) \\
R_v&=2.94\exp(0.07T_a)m^3\times10^{-3}s^{-1}
\end{align*}
...
$$C_i = \bigg[\frac{0.217e(T_a)}{T_a}\bigg]r.h.$$
\begin{align*}
LE&=2.43J\:kg^{-1}(C_e-C_i)\cdot R_v \\
&=2.43r.h.\bigg[\frac{0.217e[(T_a+T_b)/2]}{(T_b+T_a)/2}-\frac{0.217e(T_a)r.h.}{T_a}\bigg]\times2.94\exp(0.07T_a) \\
\end{align*}
...
$$Q_{cond}=\frac{kA_c(T_b-T_g)}{\Delta x}$$
...
$$h_c=\frac{k}{D}0.60\bigg(\frac{VD}{\nu}\bigg)^{0.5}=\frac{0.60}{\nu^{0.5}}\times k\bigg(\frac{V}{D}\bigg)^{0.5}$$
$$h_c=3.98\bigg(\frac{V}{D}\bigg)^{0.5}$$

This is approximately 2.3 times the value of $9.14 J m^2 {}^\circ C^{-1} s^{-1/2}$ that Gates uses. Most wind speeds in natural environments are less than $3ms^{-1}$ and most leaves are less than 0.1 m along their largest dimension, thus, Re < 20,000. Leaves usually will not flutter at wind speeds less than $3 m s^{-1}$. (See figure for Problem 7 on following page.) See Grace (1977) for additional information.

```{r fig-heattrans-ps7, echo=FALSE, fig.height=4, fig.show = "hold", out.width = "100%", fig.align = "default", fig.cap = "Figure for problem 7"}
par(mar = c(5, 5, 3, 5))
V = function(D){0.302/D}
curve(V, from = 0, to = 0.3, ylim = c(0, 10), xlab = "Diameter of leaf (m)", ylab = expression("Wind velocity V " ~ (m ~ s^-1)))
text(x = 0.2, y = 6, labels = "Re > 20000")
text(x = 0.05, y = 1.5, labels = "Re < 20000")
```

8.
\begin{align*}
Re^2&=Gr \\
\frac{D^2V^2}{\nu^2}&=\frac{agD^3(T_s-T)}{\nu^2} \\
V^2&=agD(T_s-T) \\
V&=0.1895\sqrt{(T_s-T)D} \\
\end{align*}

```{r fig-heattrans-ps8, echo=FALSE, fig.height=4, fig.show = "hold", out.width = "100%", fig.align = "default", fig.cap = "Curves for which the Reynolds number squared equals the Grasnof number."}

par(mar = c(5, 5, 3, 5))
V5 = function(D){sqrt(98 * D / 273 / 2)}
V10 = function(D){sqrt(98 * D / 273)}
V30 = function(D){sqrt(98 * D / 273 * 3)}

curve(V5, from = 0, to = 0.3, ylim = c(0, 0.5), xlab = "Diameter (m)", ylab = expression("Velocity " ~ (m ~ s^{-1})))
curve(V10, from = 0, to = 0.3, add = TRUE)
curve(V30, from = 0, to = 0.3, add = TRUE)
text(x = 0.27, y = 0.17, labels = expression(T[s] ~ "- T = 5"), cex = 0.75)
text(x = 0.20, y = 0.32, labels = expression(T[s] ~ "- T = 10"), cex = 0.75)
text(x = 0.13, y = 0.43, labels = expression(T[s] ~ "- T = 30"), cex = 0.75)
...
b. For $Re=50$, $D=0.01m$, $\nu = 1.42\times10^{-5}$ and $Re=\frac{DV}{\nu}$.  
So $V=\frac{\nu Re}{D}=\frac{1.42\times10^{-5}\times50}{0.01}=0.710m/s$
...
$Re=\frac{DV}{\nu}$ so $Nu=0.34(\frac{DV}{\nu})^{0.6}$ and $h_c=\frac{0.34(\frac{DV}{\nu})^{0.6}}{D}$
Therefore, if
...
:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:
$Q_a$ (Wm^-2^)|288|431|505|576|626|667|642|604|536|459|305|290
