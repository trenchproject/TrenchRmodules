# Foundations of Physical Theory I: Force and Energy. {#physics}
...
Newton's fit3t law (law of inertia): Every body will remain in a state of uniform motion unless acted on by external force. 
...
PROBLEM 2. Determine the repulsive force between two electrons placed at a distance of 1 mm from each other.

PROBLEM 3: Determine the attractive force due to gravitation between the above two electrons (mass of electron $- 9.107 \times 10^{-31}$kg). Hence demonstrate that for electron the electrical force is about 10^42^ times as strong as the gravitational force.
...
The units for magnetic field $(B)$ will be seen from Equation \@ref(eq:5) (next page) to be nt-sec per coul-m, which is conventionally called weber/m^2^ instead.
...
PROBLEM 4: Compute the total magnetic field at the point in Fig.\@ref(fig:fig-physics-4) by summing (integrating) the contribution from all elements $dl$, in a very long wire carrying current $i$. (Assume the point is a perpendicular distance $R$ from the wire.)  
...
###Other Force "Laws"--Friction, Intermolecular Forces, Hooke's Law
...
```{r fig-physics-8, echo=FALSE, fig.height=4, out.width = "75%", fig.cap='Relations between F, $\theta$, and ds used in computing work in moving an object from A to B.'}
...
$$W=mgsin\phi\frac{h}{sin\phi}=mgh$$
...
We may insert this into the definition of work (Equation \@ref(eq:8)) to get:
\begin{equation}
...
All that need be done is to return to the development of the formula \@ref(eq:11) for the kinetic energy, which was defined as the work necessary to get a body from zero velocity to a velocity $v$. If we wish to find the work done *on* the body in getting from a velocity $v_1$ (at point 1 in space) to a velocity $v_2$ (at point 2 in space), the steps are the same as in obtaining equation \@ref(eq:11) except the final integral has limits $v_1$ and $v_2$:
...
Comparing this to equation \@ref(eq:14) gives us the statement of conservation of energy:
$$V_1-V_2=T_2-T_1$$
or
\begin{equation}
...
We can now establish formulas for gravitational and electrostatic potential energy. For reasons that will become apparent in the development, we usually choose $r = \infty$ to be the reference point for the potential energy; that is, we shall define $V_{\infty} = 0$. Thus if we take equation \@ref(eq:15) and \@ref(eq:8) in combination to define potential energy, then substitution of equation \@ref(eq:2), Newton's law of universal gravitation, into the defining potential energy relations gives:
\begin{eqnarray}
...
PROBLEM 6: Show that $\frac{GM}{R^2}=g$, the acceleration due to gravity at the surface of the earth. Hence show that the escape velocity is approximately seven miles per second, knowing that $g$ = 9.81 m/s^2^ and $R$ = 6368 km.
