...
path<- '/Volumes/GoogleDrive/Shared drives/TrEnCh/TrEnChTutorials/data/microclim_Aust/' # put your path here for microclim data folder for Australia

solar_jan_Aust<-brick(paste0(path,"solar_radiation_Wm2/SOLR_1.nc")) # read the January solar radiation file into memory
solar_jul_Aust<-brick(paste0(path,"solar_radiation_Wm2/SOLR_7.nc")) # read the July solar radiation file into memory
Tair_jan_Aust<-brick(paste0(path,"air_temperature_degC_1cm/soil/0_shade/TA1cm_soil_0_1.nc")) # read the January 1cm air temperature in 0% shade file into memory
Tair_jul_Aust<-brick(paste0(path,"air_temperature_degC_1cm/soil/0_shade/TA1cm_soil_0_7.nc")) # read the July 1cm air temperature in 0% shade file into memory
Tsky_jan_Aust<-brick(paste0(path,"sky_temperature_degC/0_shade/TSKY_0_1.nc")) # read the January sky temperature in 0% shade file into memory
Tsky_jul_Aust<-brick(paste0(path,"sky_temperature_degC/0_shade/TSKY_0_7.nc")) # read the January sky air temperature in 0% shade file into memory
D0cm_jan_Aust<-brick(paste0(path,"substrate_temperature_degC/soil/0_shade/D0cm_soil_0_1.nc")) # read the January surface temperature in 0% shade file into memory
D0cm_jul_Aust<-brick(paste0(path,"substrate_temperature_degC/soil/0_shade/D0cm_soil_0_7.nc")) # read the July surface temperature in 0% shade file into memory
```
...
```{r}
# Computes climate space following the equations of Porter and Gates (1969), using the microclim layers as input

# inputs
# hg, A grid of estimated solar radiation (direct and diffuse, i.e. 'global') flux on horizontal ground, W/m2
...
Qbound_micro <-
  function(hg = hg, Tsub = Tsub, Tsky = Tsky, alpha_s = 0.8, alpha_g = 0.8, pctdiff = 0.15) {
    ...
    Q_rad <- (alpha_s * hS * (2 / pi) + alpha_s * hd + alpha_s * r * hg + Q_sky + Q_ground) / 2 # eq. 13 of Porter and Gates 1969
  }
```
...
RadTair <- cbind(values(Q_rad_jan_Aust[[i]]),values(Tair_jan_Aust[[i]])) # extract the radiation and air temperatures from the rasters and bind them together into a table
plot(RadTair[,1],RadTair[,2],ylim = c(-40,60), xlim = c(0,1300), pch=16, col = 'blue', xlab = 'radiation, W/m2', ylab = 'air temperature, deg C', main = "available climate space, January")
...
points(RadTair[,1],RadTair[,2],ylim = c(-40,60), xlim = c(0,1300), pch=16, col = 'red', xlab = 'radiation, W/m2', ylab = 'air temperature, deg C', main = "available climate space, January")
```
...
# compute the total radiation load
Q_rad_jul_Aust <- Qbound_micro(hg = hg, Tsub = Tsub, Tsky = Tsky, alpha_s = alpha_s, alpha_g = alpha_g)
...
for(i in 1:24) { # start of the loop, from i = 1 to 24
  ...
  if(i == 1){ # check if it is the first step, and make the first plot
    plot(RadTair[,1],RadTair[,2],ylim = c(-40,60), xlim = c(0,1300), pch = 16, col = 'red', xlab = 'radiation, W/m2', ylab = 'air temperature, deg C', main = "available climate space")
  }else{
  ...
  RadTair <- cbind(values(Q_rad_jul_Aust[[i]]),values(Tair_jul_Aust[[i]])) # extract the radiation and air temperatures from the rasters and bind them together into a table
...
for(i in 1:24) { # start of the loop, from i = 1 to 24
  ...
  if(i == 1){ # check if it is the first step, and make the first plot
    plot(RadTair[,1],RadTair[,2],ylim = c(-40,60), xlim = c(0,1300), pch = 16, col = 'red', xlab = 'radiation, W/m2', ylab = 'air temperature, deg C', main = "available climate space, Australia")
  }else{
    # it's not the first step, so use 'points' to add data to the plot
...
```{r, echo = FALSE, fig.width = 7, fig.height = 6}
path<- '/Volumes/GoogleDrive/Shared drives/TrEnCh/TrEnChTutorials/data/microclim_NAm/' # put your path here for microclim data folder for North America

solar_jan_NAm<-brick(paste0(path,"solar_radiation_Wm2/SOLR_1.nc")) # read the January solar radiation file into memory
solar_jul_NAm<-brick(paste0(path,"solar_radiation_Wm2/SOLR_7.nc")) # read the July solar radiation file into memory
Tair_jan_NAm<-brick(paste0(path,"air_temperature_degC_1cm/soil/0_shade/TA1cm_soil_0_1.nc")) # read the January 1cm air temperature in 0% shade file into memory
Tair_jul_NAm<-brick(paste0(path,"air_temperature_degC_1cm/soil/0_shade/TA1cm_soil_0_7.nc")) # read the July 1cm air temperature in 0% shade file into memory
Tsky_jan_NAm<-brick(paste0(path,"sky_temperature_degC/0_shade/TSKY_0_1.nc")) # read the January sky temperature in 0% shade file into memory
Tsky_jul_NAm<-brick(paste0(path,"sky_temperature_degC/0_shade/TSKY_0_7.nc")) # read the January sky air temperature in 0% shade file into memory
D0cm_jan_NAm<-brick(paste0(path,"substrate_temperature_degC/soil/0_shade/D0cm_soil_0_1.nc")) # read the January surface temperature in 0% shade file into memory
D0cm_jul_NAm<-brick(paste0(path,"substrate_temperature_degC/soil/0_shade/D0cm_soil_0_7.nc")) # read the July surface temperature in 0% shade file into memory
...
for(i in 1:24) { # start of the loop, from i = 1 to 24
  ...
  # first do January
  RadTair <- cbind(values(Q_rad_jan_NAm[[i]]),values(Tair_jan_NAm[[i]])) # extract the radiation and air temperatures from the rasters and bind them together into a table

  if(i == 1){ # check if it is the first step, and make the first plot
    plot(RadTair[,1],RadTair[,2],ylim = c(-40,60), xlim = c(0,1300), pch = 16, col = 'red', xlab = 'radiation, W/m2', ylab = 'air temperature, deg C', main = "available climate space, North America")
  }else{
    # it's not the first step, so use 'points' to add data to the plot
    points(RadTair[,1],RadTair[,2], col = 'grey', pch = 16)
  }
  ...
  # next do July
  RadTair <- cbind(values(Q_rad_jul_NAm[[i]]),values(Tair_jul_NAm[[i]])) # extract the radiation and air temperatures from the rasters and bind them together into a table
...
for(i in 1:24) { # start of the loop, from i = 1 to 24
  ...
  if(i == 1){ # check if it is the first step, and make the first plot
    plot(RadTair[,1],RadTair[,2],ylim = c(-40,60), xlim = c(0,1300), pch = 16, col = 'red', xlab = 'radiation, W/m2', ylab = 'air temperature, deg C', main = "Desert Iguana climate space, North America")
  }else{
    # it's not the first step, so use 'points' to add data to the plot
    points(RadTair[,1],RadTair[,2], col = 'grey', pch = 16)
  }
...
```{r}
# function to compute absorbed radiation required given a specific core temperature, air temperature and wind speed
...
```{r}
path<- '/Volumes/GoogleDrive/Shared drives/TrEnCh/TrEnChTutorials/data/microclim_NAm/' # put your path here for microclim data folder for Australia
wind_jan_NAm<-brick(paste0(path,"wind_speed_ms_1cm/V1cm_1.nc")) # read the January wind speed file for North America into memory
wind_jul_NAm<-brick(paste0(path,"wind_speed_ms_1cm/V1cm_7.nc")) # read the July wind speed  file for North America into memory
solar_jan_NAm<-brick(paste0(path,"solar_radiation_Wm2/SOLR_1.nc")) # read the January solar radiation file into memory
solar_jul_NAm<-brick(paste0(path,"solar_radiation_Wm2/SOLR_7.nc")) # read the July solar radiation file into memory
Tair_jan_NAm<-brick(paste0(path,"air_temperature_degC_1cm/soil/0_shade/TA1cm_soil_0_1.nc")) # read the January 1cm air temperature in 0% shade file into memory
Tair_jul_NAm<-brick(paste0(path,"air_temperature_degC_1cm/soil/0_shade/TA1cm_soil_0_7.nc")) # read the July 1cm air temperature in 0% shade file into memory
...
```{r}
# compute the total radiation load for each air temperature and wind speed combination that would give a Tb of 3 degrees, the lower survivable limit
Q_abs_lower <- Qabs_ecto(D = D, T_b = T_b_lower, M = M, E_ex = E_ex, E_sw = E_sw, K_b = K_b, epsilon = epsilon, T_air = T_air, V = V)
```
...
# compute the total radiation load for each air temperature and wind speed combination that would give a Tb of 45 degrees, the upper survivable limit
Q_abs_upper <- Qabs_ecto(D = D, T_b = T_b_upper, M = M, E_ex = E_ex, E_sw = E_sw, K_b = K_b, epsilon = epsilon, T_air = T_air, V = V)

enbal_hot <- Q_rad_jul_NAm - Q_abs_upper # subtract available radiation from required for 45C degrees Tb
```
...
values(enbal_hot_binary) <- ifelse(values(enbal_hot) > 0, 0, 1) # replace values greater than zero, i.e. where there is too much radiation available to match the level needed to produce a 45 degree C body temperature for each wind speed and air temperature combination across North America, otherwise give value 1

enbal_cold_binary <- enbal_cold
values(enbal_cold_binary) <- ifelse(values(enbal_cold) > 0, 0, 1) # replace values greater than zero, i.e. where there isn't enough radiation available to match the level needed to produce a 3 degree C body temperature for each wind speed and air temperature combination across North America, otherwise give value 1
