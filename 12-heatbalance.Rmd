...
**Notes**  
Up until here is good. $Re = \frac{v d}{\nu}$, with $v=0.2778$, $d=0.32$ and $\nu=1.42 \times 10^{-5}$.
$Nu = 0.17Re^{0.62}$ and it produces the exact value as listed.
...
For $T_a = 30 C$ and $d = 0.5 m$, $A_h = 1.58 \approx \pi/2$ (which is what it is specified as in the previous problem so could be the correct value?).
Proving that either the equation or the answer is wrong.
...
Solving for $T_h$ using $T_h=\frac{\alpha T_a+\beta T_b}{\alpha+\beta}+\frac{R_{Na}} {\alpha+\beta}$ with $\alpha = 17.387$, $\beta = 4.43$, $T_a = 2.92$, $T_b = 39$ (both converted to kelvin. won't work as celcius either) and $R_{Na} = -38.08$ won't even produce the listed value of -1.85.

Maybe $T_a$ or $T_b$ is wrong?  

I solved for $T_a$ and $T_b$ using $T_h=\frac{\alpha T_a+\beta T_b}{\alpha+\beta}+\frac{R_{Na}} {\alpha+\beta}$, and the values listed would only make sense if $T_a = -6.5 °C$ and $T_b = 23.6 °C$.

At the same time, it says $\beta(T_b-T_h) = 180.96$ when $\beta = 4.43$ and $T_h = -1.85$, making $T_b = 39 °C$ correct.
...
```
Notes:  
How was 341 W calculated? A_h isn't provided, and there are two options for d.
A_h = 1.12 must have been used to get 341 W if d = 0.32.
