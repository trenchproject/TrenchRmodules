...
```{r fig-heatbalance-1, echo=FALSE, fig.height=5, fig.width=5, fig.cap='Sheep idealized as a cylinder. In the diagram, T~a~, T~b~, T~s~, T~h~ and T~f~ are the temperatures of the air, body, skin, hair tips and fat; r~b~,r~s~, and r~h~ the corresponding radii of the cylinders; and K~a~, K~b~ and K~f~ the thermal conductivities of air,fleece and fat.'}
...
Incorporating the constant in \@ref(eq:5) gives 
\begin{equation}
\frac{Qr}{kA}= T_b-T_r.
(\#eq:6)
\end{equation}
...
Incorporating the concepts of resistance into \@ref(eq:6) allows us to express resistance perpendicular to a surface area as
\begin{equation}
Z= \frac{r}{kA}.
(\#eq:8)
\end{equation}
...
Substituting these results in \@ref(eq:14), we obtain
...
```{r fig-heatbalance-3, echo=FALSE, fig.height=5, fig.width=5, fig.cap='Temperature profile across fleece and air. Delta represents boundary layer thickness, T~h~=fleece-tip temperature, T~a~ = air temperature.'}
...
## Heat Gained by Absorpotion of Radiation
...
```{r fig-heatbalance-2, echo=FALSE, fig.height=5, fig.width=5, fig.cap='Energy gained by sheep from envrionment. Symbols are as follows: S = direct solar radiation, s = sky radiation, R~s~ = reflected solar radiation; R~g~, R~a~ = long-wave (thermal) radiation from ground and air, respectively.'}
...
```
Notes:  
Should T_h=10°C=283K since in the calculations, the inserted value is 283?  
It's like this in the original module but most likely a typo.
```
...
$$M-E+R_{Nh}=H+C$$
If we ignore heat loss through sweat (which can be relatively low in cool air temperatures), we can use Eqn. \@ref(eq:17) to obtain,
\begin{equation}
...
## PROBLEMS
1. Brockway et al. (1965. Evaporative heat-loss mechanisms in sheep. J. Physiology 179: 554-568) compared the contributions of metabolism and evaporative water loss in sheep. Use the model in this modules to estimate the total and evaporative heat losses at three temperatures (10, 20, 30 °C) and three body diameters ($r_h$=0.32, 0.36, 0.50) corresponding to fur thicknesses (Also, $r_s$=0.3). Then roughly compare your results to heat losses measured by Brockway et al. We will assume windspeed $v$=0.2778 m s^-1^. You can estimate net radiation under the assumption that the sheep's body temperature is steady state such that the tips of its fleece are at air temperature. Assume the following relationships for how the kinematic viscosity of air $\nu$ and the thermal conductivity or air $k_a$ vary with temperature.
...
2. What is the total heat of metabolic origin that passes outward through the fleece tips required to maintain a $T_b$ of 39 °C given that $T_a$ = 2.92 °C for sheep with diameters of 0.32m and 0.5m? Assume, $k_a= 2.37\times10^{-2}Wm^{-1}°C^{-1}$, $\nu=1.24\times10^{-5}m^2s^{-1}$, $v=2.778ms^{-1}$, and evaporative heat losses $E = 20.4W$.
[Please check calcuations. Might change solution numbers so that T_b=3C.]
...
4. Consider a sheep on a sunny day experiencing direct solar radiation of 800W and diffuse sky radiation of 150W. Suppose your sheep is standing in a pasture in which reflectivity of the grass is 25 percent. Wind speed is 1 km hr^-1^. Assume cutaneous evaporative heat loss equals 4 kcal kg^-1^ day^-1^, but respiratory heat loss can vary. It is known that sheep can breathe as few as 20 times or as many as 250 times per minute, and the corresponding respiratory heat loss varies from 2.0 to 18.0 kcal kg^-1^ day^-1^. Calculate the rate of heat loss if the sheep is shorn (fleece 1 cm thick) and unshorn (fleece 10 cm thick). Assume that total metabolic heat produced is 1900 kcal day^-1^. What does the comparison of these heat fluxes suggest about whether evaporative water loss will be sufficient for the sheep to avoid overheating in the sun?
**Likely needs more fixing**

5. Now assume that instead of a sheep your study animal is a blue whale, which you can idealize as a cylinder 25 m long and 5 m in diameter, with a layer of blubber 50 cm thick. Its body temperature is maintained at 33°C. If the whale swims at a rate of 15 km hr$^{-1}$ through 5°C water, how much food (in kcal day$^{-1}$) must it consume to maintain thermal equilibrium? Take the Nusselt number $Nu$ as $0.332 (Re_x)^{1/2} \stackrel{.}{=} 1.6 \times 10^4$ (since $V = 15 km \:hr^{-1}$, $x \stackrel{.}{=} 10 m$, $v = 1.7 \times 10^{-5} m^2 sec^{-1}$).
...
8. Derive equation \@ref(eq:23) in the text.
...
**Notes**  
Up until here is good. Using the equation $Re = \frac{v d}{\nu}$ and values $v=0.2778$, $d=0.32$ and $\nu=1.42 \times 10^{-5}$, we can get $Re$. Then $Nu = 0.17Re^{0.62}$ to produce the exact value as listed.

After this, $A_h$ isn't specified to I tried to calculate what it would be using what's already known.
Upon that I used the only equation in this module to get $\alpha$, which is $\alpha = A_h(\frac{Nuk_a}{d}+2\sigma T_a^3)$. On the table in the solution, $Nu$ and $\alpha$ are listed, $k_a$ is given and $\sigma$ is a constant.

For $T_a = 10 °C$ and $d=0.32 m$, $A_h$ turned out to be 1.014.  
For $T_a = 10 °C$ and $d = 0.50 m$, $A_h$ is 1.069. and  
For $T_a = 30 °C$ and $d = 0.50 m$, $A_h = 1.58 \approx \pi/2$ (which is what it is specified as in the previous problem so could be the correct value?).  
This proves that either the equation or the answer is wrong. Or we are simply missing an equation to get $\alpha$ more easily.

If we are only limited to the equation above to get $\alpha$, I would provide a new value of $A_h$ and update the table to match the answer.  

In terms of $\beta$, it's more complicated because the only equation to get it is $\beta=1/(Z_h+Z_f)$, and $Z_f=\frac{1}{2\pi k_fL}ln(\frac{r_s}{r_b})$, $Z_h=\frac{1}{2\pi k_hL}ln(\frac{r_h}{r_s})$ are needed to get $Z_f$ and $Z_h$ unless specified.

We would need to provide $L$, $k_f$, $k_h$, $r_b$ to get it.
**End Notes**
...
**Notes**  
Here again, up to Nusselt number calculation is good. To get $\alpha$, $A_h$ needs to be known.

I solved for $T_h$ using $T_h=\frac{\alpha T_a+\beta T_b}{\alpha+\beta}+\frac{R_{Na}} {\alpha+\beta}$ with $\alpha = 17.387$, $\beta = 4.43$, $T_a = 2.92$, $T_b = 39$ (both converted to kelvin) and $R_{Na} = -38.08$ to make sure that the values match. The result is that it
didn't even produce the listed value of -1.85.

I thought maybe $T_a$ or $T_b$ is wrong because those are the only values that are not on the table.  
So, I solved for $T_a$ and $T_b$ using $T_h=\frac{\alpha T_a+\beta T_b}{\alpha+\beta}+\frac{R_{Na}} {\alpha+\beta}$ and the parameters for two scenarios. I got $T_a = -6.5 °C$ and $T_b = 23.6 °C$.

At the same time, the table says $\beta(T_b-T_h) = 180.96$ when $\beta = 4.43$ and $T_h = -1.85$, making $T_b = 39 °C$ correct.

Overall, there seems to be issues on either the calculations they made, or the equations to be used.  
We would probably need to be clear on what values we are giving and update all the answers accordingly.

**End Notes**

3. 
The conversion is 5.30g of grass day$^{-1}$ W$^{-1}$. Using the relationship for convective heat loss,
$$\frac{Nu}{d}k_aA_h(T_h-T_a)=341W,$$
we estimate the following feeding requirements:

|                            |  0.32 |  0.50 |
|----------------------------|:-----:|:-----:|
| Required M (W)             | 48.739 | 76.03 |
| Required grass (g day^-1^) |  258.3 |  403.0  |


```
Notes:  
I am wondering how 341 W was calculated? A_h isn't provided, and there are two options for d so it shouldn't be able to be solved.  
A_h = 1.12 must have been used to get 341 W if d = 0.32 but that is pretty random.

Not sure how M was calculated either...
```
...
**Notes**  
This equation must be corresponding to 
$$R_{Na}=a_1A_1S+a_2A_2s+a_3A_3(S+s)+a_4A_4R_a+a_5A_5\sigma T_a^4-\varepsilon A_h\sigma T_a^4$$
but disregarding $a_4$ and $a_5$.  
S = 800, s = 150 are stated but we are not provided with $L$, $a_1, a_2, a_3$ (= 0.74, 0.74, 0.75 respectively), $\varepsilon$ (= 0.1396), $A_1 = dL$, $A_2 = \frac{\pi dL}{2}$, $A_3 = \frac{\pi dL}{3}$, $A_h = \pi dL$, all assuming from comparing the equations.  

The question is asking to calculate the rate of heat loss of shorn and unshorn sheep. The answer isn't answering them.  
...
## APPENDIX I. OPERATIVE ENVIRONMENTAL TEMPERATURE

**Symbols. Units and Dimensions**

|     Symbol    |                                                   Quantity                                                  |      Unit     |       Dimension      |
|:-------------:|:-----------------------------------------------------------------------------------------------------------:|:-------------:|:--------------------:|
|      $A$      |                                         Area across which heat flows                                        |      m^2^     |         L^2^         |
...
|      $B$      |                                            Sky thermal radiation                                            |    W m^-2^    |        HL^-2^        |
...
|      $k$      |                                       Thermal conductivity coefficient                                      | W m^-2^ K^-1^ | H L^-2^ $\theta$^-1^ |
|     $k_a$     |                                         Thermal conductivity of air                                         | W m^-2^ K^-1^ | H L^-2^ $\theta$^-1^ |
|     $k_f$     |                                         Thermal conductivity of fat                                         | W m^-2^ K^-1^ | H L^-2^ $\theta$^-1^ |
|     $k_h$     |                                    Thermal conductivity of hair (fleece)                                    | W m^-2^ K^-1^ | H L^-2^ $\theta$^-1^ |
|      $L$      |                        Length of cylinder representing the body length of the animal                        |       m       |           L          |
|      $M$      |                               Rate of metabolic heat production by the animal                               |       Q       |           H          |
...
|     $R_a$     |                 Thermal radiation per unit area of animal surface originating in atmosphere                 |    W m^-2^    |        H L^-2^       |
|     $R_g$     |  Thermal radiation per unit area of animal surface  originating from earth's surface (including vegetation) |    W m^-2^    |        H L^-2^       |
...
|      $S$      |                         Direct solar radiation per unit area impinging on the animal                        |    W m^-2^    |        H L^-2^       |
|      $s$      |               Sky (scattered and reflected solar) radiation per unit area impinging on animal               |    W m^-2^    |           H          |
|      $T$      |                                                 Temperature                                                 |       K       |       $\theta$       |
|     $T_b$     |                                          Body temperature of animal                                         |       K       |       $\theta$       |
|     $T_h$     |                        Surface, "radiant or hair (fleece-tip)  temperature of animal                        |       K       |       $\theta$       |
|     $T_s$     |                                          Skin temperature of animal                                         |       K       |       $\theta$       |
|      $v$      |                                                  Wind speed                                                 |    m s^-1^    |        L T^-1^       |
|     $Z_f$     |                                   "Impedance" to flow of heat through fat                                   |   s K J^-1^   |    $\theta$ W^-1^    |
|     $Z_h$     |                                   "Impedance" to flow of heat through hair                                  |   s K J^-1^   |    $\theta$ W^-1^    |
|    $\alpha$   |                            "Conductance" associated with radiation and convection                           |    W °C^-1^   |         W^-1^        |
|    $\beta$    |                       "Conductance" associated with animal's insulation (fat and hair)                      |    W °C^-1^   |         W^-1^        |
|    $\delta$   |                                           Boundary layer thickness                                          |       m       |           L          |
| $\varepsilon$ |                                                  Emissivity                                                 | dimensionless |          ---         |
|     $\nu$     |                                             Kinematic viscosity                                             |   m^2^ s^-1^  |      L^2^ T^-1^      |
|    $\sigma$   |                                          Stefan-Boltzmann constant                                          | W m^-2^ K^-4^ | H L^-2^ $\theta$^-4^ |
