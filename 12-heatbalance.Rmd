...
Once we have $Nu$ $\alpha$ can be obtained from $\alpha=A_h(Nuk_a/d+2\sigma T_a^3)$ \@ref(eq:28). The calculations for $\beta$ is $\beta=1/(Z_h+Z_f)$ \@ref(eq:29). $Z_f$ and $Z_h$ to get $\beta$ are calculated from \@ref(eq:10) and \@ref(eq:11).
...
Using $\alpha$, $\beta$, $T_a$, $R_{Na}$ and $T_b = 39°C$, $T_h$ can be calculated from $T_h=\frac{\alpha T_a+\beta T_b}{\alpha+\beta}+\frac{R_{Na}} {\alpha+\beta}$ \@ref(eq:27).
...
From equation \@ref(eq:17), the total heat loss can be estimated at $M$ since $T_b$ is always greater than $T_h$. 
