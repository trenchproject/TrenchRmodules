...
(\#eq:2)
\end{equation}
where $V$ is the wind speed in cm/sec, $r$ is the radius of the layer in cm, and $k$ is a constant describing thermal conductivity. The thermal conductivities of the fat ($k_f$), fleece ($k_h$), and air ($k_a$) additionally determine rates of heat exchange. (See further analysis below.)
...
The temperature differences between the layers can then be expressed as the product of incoming heat and thermal resistances. The expressions for the fat and fur layers as thus as follows:
\begin{equation}
Z_f Q_f =  T_b - T_s\\
Z_h Q_h =  T_s - T_h
(\#eq:12)
...
\begin{eqnarray}
Nu &=& H_L * d / k\\
194 &=& H_L * 0.5 / (2.57 * 10^{-2})\\
H_L=10\\
H&=& H_L A_h(T_h-T_a)\\
H&=& 10* 5\pi*10^{-1}*(25-10)\\
H&=&235 W
\end{eqnarray}
assuming $T_h$ = 25°C and $T_a$ = 10°C.

The temperature profile associated with convection is depicted in (Figure \@ref(fig:fig-thermintro-2)).
...
$$R_\_=1* (\pi * 0.5 *1) * 5.67*x*10^{-8} * 293^4= 656 W,$$ 
assuming $\varepsilon=1$ and $T_h$= 293K and using $\sigma=5.67*x*10^{-8}$ $W M^{-2}K^{-4}$. 

##Heat Gained by Absorpotion of Radiation
We next consider heat gained by the absorption of radiation including direct solar and sky radiation, reflected solar radiation, and long-wave radiation from the ground and air (Figure \@ref(fig:fig-thermintro-2))
...
2. What is the total heat of metabolic origin that passes outward through the fleece tips required to maintain a $T_b$ of 39C given that $T_a$=2.92C for sheep with diameters of 0.32m and 0.5m? Assume, $k_a= 2.37\times10^{-2}Wm^{-1}C^{-1}$, $\nu=1.24\times10^{-5}m^2s^{-1}$, $v=2.778ms^{-1}$, and evaporative heat losses $E$=20.4W.
[Please check calcuations. Might change solution numbers so that T_b=3C.]

3. Use the values in problem 2 but now assume $T_h$=20C and $T_a$=0C. How much grass are sheep with diameters of 0.32m and 0.5m required to eat to offset convective heat loss if consuming 5.30g of grass per day is required to offset 1W? 
...
Nusselt Numbers Diameter (m) as a function of air temperature (C)
...
>tables 
**please add tables, noting "as a function of air temperature (C)"**
...
3. 
The conversion is 5.30g of grass day$^{-1}$ W$^{-1}$. Using the relationship for convective heat loss convective heat loss,
$$\frac{Nu}{d}k_aA_h(T_h-T_a)=341W,$$
...
>appendix 1
**PLEASE ADD APPENDIX IF REASONABLY EASY TO DO SO**
