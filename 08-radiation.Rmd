...
## Black Body Radiation {#radiation-bbradiation}
...
```{r fig-radiation-1, echo=FALSE, fig.height=4, fig.show = "hold", out.width = "100%", fig.align = "default", fig.cap='The blackbody radiation predicted from Plancks law closely aligned with that empirically observed (e.g., Lummer and Pringsheim 1901).'}
...
### Wein's Law of Shift {#radiation-weins}
...
```{r fig-radiation-2, echo=FALSE, fig.height=4, fig.show = "hold", out.width = "100%", fig.align = "default", fig.cap='Wiens law- observed vs. predicted'}
...
### Stefan-Boltzmann Law {#radiation-sblaw}
...
can be obtained by numerical integration. This is a technique for approximating a definite integral by a finite sum. The value of the integral in Equation \@ref(eq:radiation-3) can be found to be:
...
### The solar constant {#radiation-solarconst}
...
Inserting Equation \@ref(eq:radiation-5prime) into Equation \@ref(eq:radiation-8) gives immediately:
...
We can use the above equation equationand the following physical parameters to calculate the solar constant:
\begin{align*}
T_s&=5700K \\
r_s&=7.1\times10^8m \\
...
$E_E=1.36\times10^3 Wm^{-2}s^{-1}

When we convert to $cal\cdot cm^{-2}min^{-1}$, our solar constant is $1.95 cal\cdot cm^{-2}min^{-1}$.

## Resolution {#radiation-resolution}
...
```{r fig-rad-3, echo=FALSE, fig.height=4, fig.show = "hold", out.width = "100%", fig.align = "default", fig.cap='The Rayleigh criterion: Fresnel diffraction by circular aperture. '}
...
```{r fig-rad-4, echo=FALSE, fig.height=4, fig.show = "hold", out.width = "100%", fig.align = "default", fig.cap='Insect eye - ommatidial diameter vs. the squareroot of eye radii. Points are 27 observations on hymenoptera (Barlow 1952).'}
knitr::include_graphics('figures/fig-rad-4.png')
```

## The Doppler Effect {#radiation-doppler}
...
```{r fig-rad-5, echo=FALSE, fig.height=4, fig.show = "hold", out.width = "100%", fig.align = "default", fig.cap='The source $S$, moving with velocity $V_S$ is emiting sound at a constant frequency, $f_S$. The wave fronts of this sound, which travel radially outward at velocity $C_S$, are compressed together (wavelength equals $\\lambda_F$) in front of the source and spread out (wavelength equals $\\lambda_B$) behind the source. By the time the wavefront emitted by the source at $x_0$ at time t = 0 has reached a radius of $C_ST$, the source has moved to a position $x_T$. The listner $L$ moving with velocity $V_L$ will hear the sound at a different frequency than as actually emitted by the source because it encounters wave fronts which are $\\lambda_B$ apart. An exact derivation is given in the text. [Adapted from Young 2013 p. 534.]'}
...
The thrust of the discussion presented in this section is perhaps most striking when viewed in terms of the advantages and constraints that the utilization of echolocation has conferred on the evolution of the Chiroptera. The development of echolocation has allowed the bat to utilize a previously unexploited. resource, night flying insects, but in turn has placed a premium on those bats that are able to emit high frequencies in short pulses, that can detect faint echoes, and that can detect relatively small shifts in frequency between outgoing and incoming pulses. The selection pressure for these attributes is the force responsible for numerous remarkable physical and behavioral adaptations. Our understanding of this system is by no means complete; echolocation remains a fertile territory for experiment and theory.
...
## Problem Set {#radiation-problems}
...
area of $1000 cm^2$, what is the heat loss rate if the environment is at a (radiation) temperature ($T_a$) of -10°C? Convert your answer to calories/minute. [See Gates 1968 for some actual organismal temperature measurements.]
...
## Answers to the Problem Set {#radiation-answers}
...
**Problem 3**
a) $H\approx22W$
b) $H\approx25.3W=362 \mbox{ cal/min}$

**Problem 4**
$d$ = separation of pits $\approx$ 6cm.

## Literature Cited {#radiation-litcited}
