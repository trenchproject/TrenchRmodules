...
A more complicated example is the complete expression for \@ref(eq:4) . The level of activity is based on the fraction of the maximal filtration rate. Then $m=0$ represents the "standard" state, $m=1$ gives the "active" state, and $m=0.4$ is the "routine" state . Consider oxygen consumption during summer.  
Then
$$a(m)=1.87(m+1)$$
and \@ref(eq:3) becomes
\begin{equation}
\frac{\partial C}{\partial t}=1.87(m+1)W^{(0.7)}
(\#eq:6)
\end{equation}
Changes in "$b$" are not significant so that an average value, 0.7, can be used. Since \@ref(eq:6) involves four variables, and thus cannot be plotted, we revert to the notation of \@ref(eq:2), i.e., $Q=\frac{\partial C}{\partial t}$. Then
\begin{equation}
Q=1.87(m+1)W^{(0.7)}
(\#eq:7)
\end{equation}
This last expression is similar in form to \@ref(eq:5) and its graph has a similar shape. Problem 3 discusses \@ref(eq:7) in more detail.
...
3. To convert \@ref(eq:7) so $Q$ is in ml/hr, we divide by 1000:
