...
where $r_a$ is given by Equation \@ref(eq:72). Here, if $_s d_l (T_l)$ and $_s d_a (T_a)$ are given in kg m^-3^ and $r_l$, and $r_a$ in s m^-1^, then $E$ is in kg m^-2^s^-1^. 

## COMPLETE ENERGY BUDGET

By substituting Equation (\@ref(eq:73)) into Equation (\@ref(eq:71)), and including the expression given by Equation (\@ref(eq:72)) in Equation (\@ref(eq:73)), one gets for the full expression of the leaf energy budget the following: 
\begin{equation} 
...
The influence of transpiration is now readily seen by including the transpiration term in the full energy budget expression as given in Equation \@ref(eq:74). Now one must decide on a value for the internal resistance of a leaf and the relative humidity of the air. Let $r_l = 100 s m^{-1}$ and $r.h. = 50$%. It is more difficult to solve Equation \@ref(eq:74) then it is to solve Equations \@ref(eq:75) or \@ref(eq:76). Graphical methods are possible although cumbersome. It is far easier to use a calculator or a computer and this is now done. The values are listed in Table I.
...
Let us begin by specifying $Q_a = 800 W m^{-2}, T_a = 40°C, r.h. = 0.20, D = 0.05 m, W = 0.05 m, r_a = 0.0s m^{-1}$, and $V = 0.10 m s^{-1}$. Now all the parameter values of Equation \@ref(eq:77) are known except leaf temperature. We solve for leaf temperature and then calculate transpiration rate using the last term of equation \@ref(eq:77). We examine the influence of varying wind speed and leaf resistance on transpiration as follows:
...
L=2.26*10^6 #L, latent heat of vaporiation of water in J kg^{-1}, 2.43x10^6 at 30°C and 2.5*10^6 at 0°C
