...
## PREFACE
This module introduces two models of the thermal energy budget of a leaf. Typical values for environment variables and leaf parameters are discussed and simple calculations are made to demonstrate how radiation, convection, and transpiration affect leaf temperature. A graphical method of analysis is used to present a more detailed energy budget model. A problem set and an accompanying computer program permit the student to explore the consequences of the models. Algebra and some knowledge of heat transfer physics are prerequisites.

The reasoning behind the simple, heat energy budget of a leaf is developed in this module into a detailed explanation of leaf transpiration. Although the mathematical model is conceptually simple, seven independent variables prevent easy exploration of the consequences of the model. The accompanying computer program provides students with a tool to concentrate their efforts on the biology of heat and water balance of a leaf rather than time-consuming numerical calculations. Algebra and some knowledge of heat transfer physics are prerequisites.
...
```{r fig-transpiration-1, echo=FALSE, fig.height=4, fig.fullwidth=FALSE, fig.cap='Net photosynthesis versus leaf temperature at high and low carbon dioxide concentration. From Locomen et al. 1975.  P. 41.'}
...
The graphical method of analysis used by Gates (1968) is then employed to understand a more detailed energy budget model. Problems with accompanying solutions are followed by an appendix on symbols, units and dimensions. 
...
where $r_a$ is given by Equation \@ref(eq:72). Here, if $_s d_l (T_l)$ and $_s d_a (T_a)$ are given in $kg m^{-3}$ and $rl2$, and $r_a$ in $s m^{-1}$, then $E$ is in $kg m^{-2} s^{-1}$. 

## COMPLETE ENERGY BUDGET

By substituting Equation (\@ref(eq:3)) into Equation (\@ref(eq:1)), and including the expression given by Equation (\@ref(eq:2)) in Equation (\@ref(eq:3)), one gets for the full expression of the leaf energy budget the following: 
\begin{equation} 
...
If the air temperature is $30^{\circ}C$, one can determine the exact numerical relationship between $Q_a$ and $T_l$ for any given ratio $V/D$. For example, we will determine values of $T_l$ for $Qa$ = 800, 600, and 400 W m^-2^ for $V/D$ = 1.0, 10.0, and 100 below. These values are listed in Table I.
...
rc_root= function(VDratio1, Q_a1){
  f <- function(T_l, VDratio=VDratio1, Q_a=Q_a1, T_a=30) epsilon *sigma*(T_l + 273)^4 + k1*(VDratio)^0.5*(T_l-T_a) - Q_a
uniroot(f, interval=c(-100, 100))$root
  }

rc_mat= rbind( sapply( c(1,10,100), FUN=rc_root, Q_a1=800), sapply( c(1,10,100), FUN=rc_root, Q_a1=600), sapply( c(1,10,100), FUN=rc_root, Q_a1=400) )
...
```{r}
knitr::kable(
  rc_mat, booktabs = TRUE,
  caption = 'Leaf temperature.'
)
```
...
>Table 1. Leaf temperature in °C for various amounts of absorbed radiation exchanged by radiation only, by radiation and convection, and by radiation, convection, and transpiration. Values of variables used are $Ta=30°C$, r.h.=50%, and $r_l$ = 100 s m^-1^. The addition of convective exchange to the energy budget will decrease the difference between leaf temperature and air temperature compared to the leaf temperature computed when only radiation is included. As the ratio $V/D$ increases, the magnitude of the difference between leaf temperature and air temperature decreases. Note that if absorbed radiation levels are above black body value (for $Q_a$ = 800 and 600 W m^-2^ because here black body level is $Q_a=\sigma T_a= 5.67\times 10^{-8} \times (273+30)^4 = 478 W m^{-2}$. $T_l-T_a$ is positive and if $Q_a$ is below the black body value $T_l$ -$T_a$ is negative. If transpiration is included, it will reduce all leaf temperatures from the values computed when only radiation and convection transfer are included.

For $V/D = 1.0$, simply placing the non-transpiring leaf in air of temperature 30°C dropped the leaf temperature from 75.2 to 61.5°C at $Q_a = 800 W m^{-2}$, a decrease of 13.7°C. Clearly, convection may play a very strong role in influencing leaf temperature. If $V/D = 10.0$, the decrease is, of course, even more dramatic. At $Q_a = 800 W m^{-2}$ and $T_a$ = 30°C, $T_l$ = 39.7°C, there is a decrease of $35.5^{\circ}C$ from the pure radiation regime. If $V/D = 100$ and $Q_a = 800 W m^{-2}$, the leaf temperature is 33.5°C with convection, a change of 41.7°C.
...
The influence of transpiration is now readily seen by including the transpiration term in the full energy budget expression as given in Equation \@ref(eq:4). Now one must decide on a value for the internal resistance of a leaf and the relative humidity of the air. Let $r_l = 100 s m^{-1}$ and $r.h. = 50$%. It is more difficult to solve Equation \@ref(eq:4) then it is to solve Equations \@ref(eq:5) or \@ref(eq:6). Graphical methods are possible although cumbersome. It is far easier to use a calculator or a computer and this is now done. The values are listed in Table I.
...
Gates elected to include W in the evaporation term only since the amount of data was not sufficient to reveal the functional relationship in both the convective and evaporative terms simultaneously. Equation \@ref(eq:77) is more realistic than Equation \@ref(eq:74), which, nevertheless, is a good approximation. If the wind is blowing across the leaf in the direction of the leaf width, then, the characteristic dimension $D$ is the average leaf width and $W$ is the average length. If, on the other hand, the wind is blowing along the length of the leaf, then, $D$ is the average length and $W$ is the average leaf width.

## ANALYSIS OF A MORE GENERAL MODEL
###Calculations of Leaf Temperatures and Transpiration

It is straightforward to obtain values of the two dependent variables, leaf temperatures and transpiration rate, as a function of any conceivable combination of values of the independent variables and leaf parameters which appear in Equation \@ref(eq:77). The four independent variables used are radiation absorbed, air temperature, relative humidity of the air, and wind speed; and the three leaf parameters are characteristic dimension, leaf width and internal resistance. Gates and Papian (1971) published a tabulation of leaf temperatures and transpiration rates for various sets of values of the dependent variables and leaf parameters. Similar plots are shown in a paper by Gates (1968). These plots have proved useful in understanding Equation \@ref(eq:77), and therefore at this time we will undertake an explanation of how they are produced.

Let us begin by specifying $Q_a = 800 W m^{-2}, T_a = 40^{\circ}C, r.h. = 0.20, D = 0.05 m, W = 0.05 m, r_a = 0.0, s m^{-1}$ and $V = 0.10 m s^{-1}$. Now all the parameter values of Equation \@ref(eq:77) are known except leaf temperature. We solve for leaf temperature and then calculate transpiration rate using the last term of equation \@ref(eq:77). We examine the influence of varying wind speed and leaf resistance on transpiration as follows:


```{r fig-transpiration-2, echo=FALSE, fig.height=4, out.width = "50%", fig.cap='Leaf transpiration verses leaf temperature.'}
...
#Vary V
fig2a=sapply( seq(0.1,6.1,0.2), FUN=est_TlTrans, T_a=40, rh=0.2, r_l=0, D=0.05, W=0.05, Q_a=800)
plot(fig2a[1,], fig2a[2,], type="l", xlab="Leaf temperature (C)", ylab= "Leaf transpiration (kg/m^2s)")

#Vary r_l
fig2b=sapply( seq(0,600,10), FUN=est_TlTrans, T_a=40, V=2.1, rh=0.2, D=0.05, W=0.05, Q_a=800)
points(fig2b[1,], fig2b[2,], type="l", col="red")

legend("topright", c("Vary windspeed", "Vary leaf resistance"), col = c("black", "red"), lty=c(1), bty = "n")
```
...
3) $Q_a = 800 W m^{-2}; T_a = 40°C; r.h. = 0.20$
4) $Q_a = 700 W m^{-2}; T_a = 30°C; r.h. = 0.20$
5) $Q_a = 700 W m^{-2}; T_a = 30°C; r.h. = 0.80$
6) $Q_a = 300 W m^{-2}; T_a = 10°C; r.h. = 0.50$

We create the plots in R:
```{r fig-transpiration-3, echo=FALSE, fig.height=4, fig.fullwidth=FALSE, fig.cap='Leaf transpiration versus leaf temperature.'}
#Vary V and r_l
#fig 3
...
```{r fig-transpiration-4, echo=FALSE, fig.height=4, fig.fullwidth=FALSE, fig.cap='Leaf transpiration versus leaf temperature.'}
#fig 4
fig.vr=sapply( seq(0,600,100), FUN=est_TlTrans, T_a=30, rh=0.2, V=0.1, D=0.05, W=0.05, Q_a=700)
plot(fig.vr[1,], fig.vr[2,], type="l", col='tomato', xlab="Leaf temperature (C)", ylab= "Leaf transpiration (kg/m^2s)", xlim=range(25,45),ylim=range(0.0,0.0003))
...
```{r fig-transpiration-5, echo=FALSE, fig.height=4, fig.fullwidth=FALSE, fig.cap='Leaf transpiration versus leaf temperature.'}
#fig 5
fig.vr=sapply( seq(0,600,100), FUN=est_TlTrans, T_a=30, rh=0.8, V=0.1, D=0.05, W=0.05, Q_a=700)
plot(fig.vr[1,], fig.vr[2,], type="l", col='tomato', xlab="Leaf temperature (C)", ylab= "Leaf transpiration (kg/m^2s)", xlim=range(25,45),ylim=range(0.0,0.0002))
...
```{r fig-transpiration-6, echo=FALSE, fig.height=4, fig.fullwidth=FALSE, fig.cap='Leaf transpiration versus leaf temperature.'}
#fig 6
fig.vr=sapply( seq(0,600,100), FUN=est_TlTrans, T_a=10, rh=0.5, V=0.1, D=0.05, W=0.05, Q_a=300)
plot(fig.vr[1,], fig.vr[2,], type="l", col='tomato', xlab="Leaf temperature (C)", ylab= "Leaf transpiration (kg/m^2s)", xlim=range(5,10),ylim=range(0.0,0.00015))
...
```{r fig-transpiration-7, echo=FALSE, fig.height=4, fig.fullwidth=FALSE, fig.cap='Leaf transpiration versus leaf temperature.'}
#Vary air temperature and resistance

#fig 7
fig.vr=sapply( seq(10,40,10), FUN=est_TlTrans, r_l=0, rh=0.2, V=0.1, D=0.05, W=0.05, Q_a=700)
plot(fig.vr[1,], fig.vr[2,], type="l", col='darkorange', xlab="Leaf temperature (C)", ylab= "Leaf transpiration (kg/m^2s)", xlim=range(25,55),ylim=range(0.0,0.0007))
...
```{r fig-transpiration-8, echo=FALSE, fig.height=4, fig.fullwidth=FALSE, fig.cap='Leaf transpiration versus leaf temperature.'}
#fig 8
fig.vr=sapply( seq(10,40,10), FUN=est_TlTrans, r_l=0, rh=0.8, V=0.1, D=0.05, W=0.05, Q_a=700)
plot(fig.vr[1,], fig.vr[2,], type="l", col='darkorange', xlab="Leaf temperature (C)", ylab= "Leaf transpiration (kg/m^2s)", xlim=range(20,50),ylim=range(0.0,0.0001))
...
```{r fig-transpiration-9, echo=FALSE, fig.height=4, fig.fullwidth=FALSE, fig.cap='Leaf transpiration versus leaf temperature.'}
#fig 9
fig.vr=sapply( seq(10,40,10), FUN=est_TlTrans, r_l=0, rh=0.5, V=2.0, D=0.05, W=0.05, Q_a=700)
plot(fig.vr[1,], fig.vr[2,], type="l", col='darkorange', xlab="Leaf temperature (C)", ylab= "Leaf transpiration (kg/m^2s)", xlim=range(15,45),ylim=range(0.0,0.0003))
...
```{r fig-transpiration-10, echo=FALSE, fig.height=4, fig.fullwidth=FALSE, fig.cap='Leaf transpiration versus leaf temperature.'}
#fig 10
fig.vr=sapply( seq(10,40,10), FUN=est_TlTrans, r_l=0, rh=0.5, V=0.1, D=0.05, W=0.05, Q_a=300)
plot(fig.vr[1,], fig.vr[2,], type="l", col='darkorange', xlab="Leaf temperature (C)", ylab= "Leaf transpiration (kg/m^2s)", xlim=range(5,30),ylim=range(0.0,0.00001))
...
3. Equation \@ref(eq:77) in the module indicates how a leaf dissipates the radiant energy it absorbs. The three terms on the right-hand side of \@ref(eq:77) correspond to reradiation; loss of sensible heat by convection; and loss of latent heat through transpiration. For each of the three situations listed below, find the relative contribution of each of these mechanisms to the dissipation of the incident radiant energy.
...
```{r}
fig.vr=sapply( seq(0,600,100), FUN=est_TlTrans, T_a=40, rh=0.5, V=0.1, D=0.001, W=0.02, Q_a=800)
plot(fig.vr[1,], fig.vr[2,], type="l", col='tomato', xlab="Leaf temperature (C)", ylab= "Leaf transpiration (kg/m^2s)", xlim=range(35,45),ylim=range(0.0,0.001))
fig.vr=sapply( seq(0,600,100), FUN=est_TlTrans, T_a=40, rh=0.5, V=2.1, D=0.001, W=0.02, Q_a=800)
points(fig.vr[1,], fig.vr[2,], type="l", col='tomato2')
fig.vr=sapply( seq(0,600,100), FUN=est_TlTrans, T_a=40, rh=0.5, V=4.1, D=0.001, W=0.02, Q_a=800)
points(fig.vr[1,], fig.vr[2,], type="l", col='tomato3')
fig.vr=sapply( seq(0,600,100), FUN=est_TlTrans, T_a=40, rh=0.5, V=6.1, D=0.001, W=0.02, Q_a=800)
points(fig.vr[1,], fig.vr[2,], type="l", col='tomato4')

fig.vr=sapply( seq(0.1,6.1,0.2), FUN=est_TlTrans, T_a=40, rh=0.5, r_l=0, D=0.001, W=0.02, Q_a=800)
points(fig.vr[1,], fig.vr[2,], type="l", col='skyblue', lty="dashed")
fig.vr=sapply( seq(0.1,6.1,0.2), FUN=est_TlTrans, T_a=40, rh=0.5, r_l=200, D=0.001, W=0.02, Q_a=800)
points(fig.vr[1,], fig.vr[2,], type="l", col='skyblue2', lty="dashed")
fig.vr=sapply( seq(0.1,6.1,0.2), FUN=est_TlTrans, T_a=40, rh=0.5, r_l=400, D=0.001, W=0.02, Q_a=800)
points(fig.vr[1,], fig.vr[2,], type="l", col='skyblue3', lty="dashed")
fig.vr=sapply( seq(0.1,6.1,0.2), FUN=est_TlTrans, T_a=40, rh=0.5, r_l=600, D=0.001, W=0.02, Q_a=800)
points(fig.vr[1,], fig.vr[2,], type="l", col='skyblue4', lty="dashed")
...
```{r}
fig.vr=sapply( seq(0.01,0.1,0.005), FUN=est_TlTrans, T_a=35, rh=0.2, V=0.1, W=1, Q_a=800, r_l=2000) #1045.6
plot(fig.vr[1,], fig.vr[2,], type="p", xlab="Leaf temperature (C)", ylab= "Leaf transpiration (kg/m^2s)")
...
#fig 3
fig.vr=sapply( seq(0,600,100), FUN=est_TlTrans, T_a=40, rh=0.2, V=0.1, D=0.05, W=0.05, Q_a=800)
plot(fig.vr[1,], fig.vr[2,], type="l", xlab="Leaf temperature (C)", ylab= "Leaf transpiration (kg/m^2s)")
fig.vr=sapply( seq(0.1,6.1,0.2), FUN=est_TlTrans, T_a=40, rh=0.2, r_l=200, D=0.05, W=0.05, Q_a=800)
...
L*(vd(T_l, rh)-rh*vd(T_a, rh))/(r_l + k2*(D^{0.3}*W^{0.20})/ V^{0.50})
...
#c)
#fig 8
fig.vr=sapply( seq(10,40,10), FUN=est_TlTrans, r_l=500, rh=0.8, V=0.1, D=0.05, W=0.05, Q_a=700)
plot(fig.vr[1,], fig.vr[2,], type="l", xlab="Leaf temperature (C)", ylab= "Leaf transpiration (kg/m^2s)", xlim=range(20,50),ylim=range(0.0,0.0001))
fig.vr=sapply( seq(0,1000,100), FUN=est_TlTrans, T_a=10, rh=0.8,V=0.1, D=0.05, W=0.05, Q_a=700)
points(fig.vr[1,], fig.vr[2,], type="l", lty="dashed")
