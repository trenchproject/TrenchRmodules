---
pagetitle: Calculus-Integration
output:
...
## Fundamentals {#calcint-fundamentals}
...
since differentiating $N_0 e^{kt}$ gives eqn. \@ref(eq:calcint-1). 

$$\frac{dN}{dt} = \frac{d(N_0 e^{kt})}{dt} = kN_0 e^{kt} = kN $$

If we write eqn. \@ref(eq:calcint-1) as
...
## The Definite Integral {#calcint-defintegral}
...
### Area Under the Curve  {#calcint-area}
...
We can use this equation to approximate the upper surface area of a *Kalmia latifolia* (mountain laurel) leaf. On average, the leaves are 8 cm long and 3 cm wide, thus a=4 and b=1.5, which we can use to solve for A. 
...
### Improper Integrals {#calcint-improper}
...
## Methods Of Integration {#calcint-methods}
...
### Substitution {#calcint-substitution}
...
### Integration by Parts {#calcint-intbyparts}
...
### Partial Fractions {#calcint-partial}
...
## Applications Of Definite Integrals {#calcint-applications}
...
### Accumulation of Changes in the Function {#calcint-accumulation}
...
### Average Change {#calcint-average}
...
### Surface Area of Revolution {#calcint-surface}
...
### Volume of Revolution {#calcint-volumerevolution}
...
### General Surface Areas {#calcint-surfarea}
...
### Error Estimation {#calcint-error}
...
No justification has yet been given for the constants in eqn. \@ref(eq:calcint-15). These, too, may be determined by a least squares norm. See additional problems 1 for details.
...
## Differential Equations {#calcint-diffequations}
...
### Separation of Variables {#calcint-separation}
...
### Residual Norm {#calcint-residual}
...
## References and Recommended Texts {#calcint-references}
...
b. Solve this differential equation for $N(t)$ so that the solution satisfies the initial condition, $N(0) = N_0$. (Hint: first separate variables. Do not forget the integration constant!) 
...
5. Write equation \@ref(eq:calcint-13) using $y(x_i) = Ax_i + B$. Evaluate the two first partial derivatives and, by setting both equal to zero, obtain two linear equations in A and B.
...
## Answers to the Problem Set {#calcint-answers}
**General**
...
$$\overline B(12)= (1/12) \int^{12}_0 5.0\times10^7 e^{-0.5t} (1-0.8e^{-0.05t})^3 dt $$
...
d. Let N(t) be the number of $C^{14}$ atoms in the wood. Then $N(t)/N_0 = 0.87$. The half life of 5568 gives $(\ln2)/k=5568$, $k= (\ln 2)/5568=0.0001245$.
...
a. Eq. \@ref(eq:calcint-a2) is:
$$A(1353.25) + B(65.90) - (3750.50) = 0$$
Eq. \@ref(eq:calcint-b2) is:
$$A(65.90) + 5B - (268.50) = 0$$
...
c. The following graph \@ref(fig:fig-calcint-p6) compares the least squares line and exponential curve of parts (a) and (b) with the data and the exponential curve in the text eqn. \@ref(eq:calcint-12). The fit of eqn. \@ref(eq:calcint-12) seems superior to that of the curve from (b).
However, the fit of

$$h = 2.642 - 0.071x$$

to the transformed data is quite good, as shown by figure \@ref(fig:fig-calcint-p6-2). Thus we conclude that a good least squares fit to transformed data does not necessarily imply a good curve fit to the original data.

```{r fig-calcint-p6, echo=FALSE, fig.height=4, out.width = "75%", fig.cap = "Comparison of fitted growth functions."}
knitr::include_graphics('figures/fig-calcint-p6.png')
```

```{r fig-calcint-p6-2, echo=FALSE, fig.height=4, out.width = "75%", fig.cap = "Least squares line of transformed data."}
knitr::include_graphics('figures/fig-calcint-p6-2.png')
```

## Additional Problems {#calcint-problems2}
...
```{r fig-calcint-c3, echo=FALSE, fig.height=4, out.width = "75%"}
knitr::include_graphics('figures/fig-calcint-c3.png')
```
...
## Solution to the Additional Problems {#calcint-answers2}
1.
...
```{r fig-calcint-cs3, echo=FALSE, fig.height=4, out.width = "75%"}
knitr::include_graphics('figures/fig-calcint-cs3.png')
```
