...
where
<ul class="list-unstyled">
<li> $M$ = metabolic rate ($W m^{-2}$), </li>
<li> $Q_a$ = absorbed radiation ($W m^{-2}$), </li>
<li> $\varepsilon$ = emissivity </li>
<li> $\sigma$ = Stefan-Boltzmann constant ($W m^{-2} K^{-4}$) </li>
<li> $T_a$ = air temperature </li>
<li> $h_c$ = convection coefficient ($W m^{-2} {^{\circ}C^{-1}}$) </li>
<li> and $E_{ex}$ = energy flux due to respiration ($W m^{-2}$). </li>
</ul>
...
> Table 9.1. Values for the climate space of the cardinal.
...
### Monteith's Idea {#climatespace-monteith}
...
<ul class="list-unstyled">
<li> $M$ = metabolism ($W m^{-2}$) </li>
<li> $E$ = water loss ($W m^{-2}$) </li>
<li> $T_o$ = surface temperature ($^\circ C$) </li>
<li> $T_e$ = effective temperature of the environment ($^{\circ}C$) </li>
<li> $\frac{C_a P}{r_{hr}}$ = equivalent conductivity ($W m^{-2} {^{\circ}C^{-1}}$) </li>
</ul>
...
## Extensions of the Climate Space Idea {#climatespace-extensions}
...
## Literature Cited {#climatespace-litcited}
...
## Problem Set {#climatespace-problems}

1. Table A provides metabolic rate, water loss, and fur and fat thicknesses for several species of animals. Using this information and Equations \@ref(eq:climatespace-10) and \@ref(eq:climatespace-11), calculate the maximum and minimum temperature differentials across the fat ($T_b - T_s$) and fur layers ($T_s - T_r$). 
...
## Answers to the Problem Set {#climatespace-answers}
...
5. The two formulae needed to calculate the convection coefficients are \@ref(eq:climatespace-6) and \@ref(eq:climatespace-7).
